...
class Heapss {
    ...
    public int decreaseKey(int index,Heapss h){
        if(index>size){
           return 0;
        }
        if(isLeaf(index)){
            return index;
              ...
              int right=index*2+1;
              int left=index*2;
              int rightIndex=decreaseKey(right,h);
              int leftIndex=decreaseKey(left,h);
...
class HeapDecrease {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < t; i++) {
            ...
            h.ar[k]=Integer.MIN_VALUE;
            h.buildMinHeapDuringInsertion(k,h.ar[k]);
            extratMin(h);
            h.printHeap();
        }
    }

    private static void extratMin(Heapss h) {
